...
tfv=deltaQ/((Vgg-Vgsio)/Rg)
print("Tiempo de fall de tension es",tfv)
...
#VGS encendido
p1=Vgg*(1-np.exp(-t[0:int(t1/deltat)]/tau1))
p2=np.ones(int((t1+tfv)/deltat)-int(t1/deltat)-1)*Vgsio
p3=Vgsio*np.ones(len(t)-int((t1+tfv)/deltat))+(Vgg-Vgsio)*(1-np.exp(-t[:len(t)-int((t1+tfv)/deltat)]/tau2))
Vgs=np.append(p1,p2)
Vgs=np.append(Vgs,p3)
#Ig encendido
p1=Vgg/Rg*np.exp(-t[0:int(t1/deltat)]/tau1)
p2=p1[-1]*np.ones(int((t1+tfv)/deltat)-int(t1/deltat)-1)
p3=p2[0]*(np.exp(-t[:len(t)-int((t1+tfv)/deltat)]/tau2))
Ig=np.append(p1,p2)
Ig=np.append(Ig,p3)
#Id encendido
p1=np.zeros(int(td/deltat))
p2=Io/tri*t[0:int(tri/deltat)]
p3=np.ones(len(t)-int(t1/deltat))*Io
Id=np.append(p1,p2)
Id=np.append(Id,p3)
#VDS encendido
p1=Vswmax*np.ones(int(t1/deltat))
p2=Vswmax-Vswmax/tfv*t[0:int(tfv/deltat)]
p3=np.zeros(len(t)-int((t1+tfv)/deltat))
Vds=np.append(p1,p2)
Vds=np.append(Vds,p3)

#L
df = pd.read_csv('on con id mal.csv',skiprows=1, names=['t', 'V(drain)',"V(gate)", "I(R1)", "Id(M1)"])
